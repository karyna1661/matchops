...
const mappings: Record<string, Mapping> = {
	MONITOR: {
		...
		buildPayload: (intent) => ({
			...
			labels: [intent.priority, "monitoring", "nominal"],
			severity: "low",
	...
	CREATE_INCIDENT: {
		...
		buildPayload: (intent) => ({
			...
			labels: [intent.priority, "incident", "warning", "drift-detected"],
			severity: intent.confidence > 0.7 ? "high" : "medium",
			priority: intent.priority,
			confidence: intent.confidence,
			reason: intent.reason,
		}),
	...
	ESCALATE: {
		...
		buildPayload: (intent) => ({
			...
			severity: "critical",
			labels: ["escalation", "critical", "drift-detected"],
		}),
		buildReason: (intent) =>
